...
$(document).on('click', function (evt) {
    ...
    if ($('.hamburguer').hasClass('open') && $('.hamburguer').hasClass('h-mobile')) {
        $('.hamburguer .img-h-btn').attr('src', 'public/img/icon_close_hamburguer.svg')
        $('.hamburguer .img-h-btn').addClass('close')
    } else {
        $('.hamburguer .img-h-btn').attr('src', 'public/img/hamburguer01.svg')
...
setInterval(function(){
    const text = $('.slick-current .banner-info h1').text()
